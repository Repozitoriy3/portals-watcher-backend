...
def run_bot():
    try:
        ...
        # если синхронно не получилось — просто логируем позже
        pass
...
# Запуск бота и Flask
def start_background_once():
    if getattr(start_background_once, "_started", False):
        return
    start_background_once._started = True
    Thread(target=run_bot, daemon=True).start()

@app.before_first_request
def _kickoff():
    # поднимем бота перед обслуживанием первого HTTP-запроса (важно для gunicorn)
    start_background_once()

if __name__ == "__main__":
    # локальный запуск или simple server на Render
    start_background_once()
    port = int(os.environ.get("PORT", "10000"))
